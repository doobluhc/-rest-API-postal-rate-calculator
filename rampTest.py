...
class Flasktest(unittest.TestCase):
    def test_args_none(self):
        ...
        response = requests.put(BASE + "calculator", {"length_unit" : None})
    ...
    def test_one_args(self):
        tester = app.test_client(self)
        response = requests.put(BASE + "calculator", {"length_unit" : "mm"})
    ...
    def test_missing_one_args(self):
        tester = app.test_client(self)
        response = requests.put(BASE + "calculator", {"length_unit" : "mm", "width" : 10.0, "length":10.0, "weight_unit": "g"})
    ...
    def test_all_args_exist_width_str(self):
        tester = app.test_client(self)
        response = requests.put(BASE + "calculator", {"length_unit" : 'mm', "width" : 'width', "length":10.0, "weight_unit": "g", "weight": 10.0})
        self.assertEqual(response.json()['message']['width'], "input json data format must include (str)length unit -> mm or inches , (float)width, (float)length, (str)weight unit -> g or ounces, (float)weight")

    def test_arg_length_unit_not_mm_or_inches(self):
        tester = app.test_client(self)
        response = requests.put(BASE + "calculator", {"length_unit" : 'm', "width" : 10.0, "length":10.0, "weight_unit": "g", "weight": 10.0})
        #print(response.json())
        self.assertEqual(response.json()['message']['length_unit'], "input json data format must include (str)length unit -> mm or inches , (float)width, (float)length, (str)weight unit -> g or ounces, (float)weight")
    
    def test_arg_weight_unit_not_g_or_ounces(self):
        tester = app.test_client(self)
        response = requests.put(BASE + "calculator", {"length_unit" : 'mm', "width" : 10.0, "length":10.0, "weight_unit": "kg", "weight": 10.0})
        #print(response.json())
